...
class TaskServiceTest {
    ...
    void shouldSuccessfullyCreateTask() {
        CreateTaskRequest request = new CreateTaskRequest("Tarefa", "Descrição", null, null);

        Task task = new Task(
                request.title(), request.description(), request.completed(),
                request.priorityLevel()
        );

        TaskResponse expectedResponse = new TaskResponse(
                1L, task.getTitle(), task.getDescription(),
                task.getCompleted(), task.getPriorityLevel()
        );
    ...
    void shouldFetchAllTasks() {
        ...
        TaskResponse r1 = new TaskResponse(
                1L, t1.getTitle(), t1.getDescription(),
                t1.getCompleted(), t1.getPriorityLevel()
        );
        TaskResponse r2 = new TaskResponse(
                2L, t2.getTitle(), t2.getDescription(),
                t2.getCompleted(), t2.getPriorityLevel()
        );
    ...
    void shouldFetchTaskById() {
        ...
        TaskResponse expectedResponse = new TaskResponse(
                ...
                task.getDescription(), task.getCompleted(), task.getPriorityLevel()
        );
    ...
    void shouldSuccessfullyUpdateTask() {
        ...
        UpdateTaskRequest request = new UpdateTaskRequest(
                "Tarefa Atualizada",
                "Descrição Atualizada", true, PriorityLevel.HIGH
        );

        TaskResponse expectedResponse = new TaskResponse(
                taskId, "Tarefa Atualizada",
                "Descrição Atualizada", true, PriorityLevel.HIGH
        );
    ...
    @Test
    @DisplayName("Deve lançar ResponseStatusException ao tentar obter todas as tarefas e nenhuma " +
            "for encontrada")
    ...
    @Test
    @DisplayName("Deve lançar ResponseStatusException ao tentar obter uma tarefa por ID " +
            "inexistente")
    ...
    void shouldThrowNotFoundWhenUpdatingNonExistentTask() {
        Long taskId = 999L;
        UpdateTaskRequest request = new UpdateTaskRequest(
                "Título", "Descrição", true, PriorityLevel.LOW);
